...
if rep_max == rep_min:
    ...
    edges_df["edge_width"] = EDGE_WIDTH_MIN + (
        (edges_df["log_reproducibility"] - rep_min) / (rep_max - rep_min)
...
for _, edge in edges_df.iterrows():
    ...
    ax.plot(
        [x1, x2], [y1, y2], [z1, z2],
        color=edge_cmap(rep_norm(edge["log_reproducibility"])),
        alpha=EDGE_ALPHA,
...
sm.set_array([])
cbar_edges = fig.colorbar(sm, ax=ax, shrink=0.58, aspect=16, pad=0.02)
cbar_edges.set_label("log10(|mean step| / (SD + 0.1) + 1)")

plt.tight_layout()
plt.show()

print("Plotted averaged 3D genotype network with stabilized reproducibility-weighted edges.")
print("Modifications applied:")
print("  - SD floor =", SD_FLOOR)
print("  - Reproducibility transformed as log10(|mean step| / (SD + floor) + 1)")
print()
print("Summary of edge reproducibility:")
print(edges_df[["raw_reproducibility", "log_reproducibility"]].describe())
