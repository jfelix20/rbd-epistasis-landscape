...
# Reproducibility interpretation thresholds on log10(|mean step| / (SD + 0.1) + 1)
DISAGREE_THRESH = 0.30   # SD roughly equals or exceeds signal -> hide
BORDERLINE_THRESH = 0.50 # faint below this, increasingly visible above it
...
rep_min = edges_df["log_reproducibility"].min()
rep_max = edges_df["log_reproducibility"].max()

# Width still scales across full range, but invisible edges won't matter
edges_df["edge_width"] = EDGE_WIDTH_MIN + (
    (edges_df["log_reproducibility"] - rep_min) / (rep_max - rep_min + 1e-12)
) * (EDGE_WIDTH_MAX - EDGE_WIDTH_MIN)

rep_norm = colors.Normalize(vmin=DISAGREE_THRESH, vmax=rep_max)
...
for _, edge in edges_df.iterrows():
    ...
    # Faint at borderline, ramps up toward strongest reproducibility
    if val < BORDERLINE_THRESH:
        alpha = 0.10
    else:
        alpha = 0.10 + 0.90 * ((val - BORDERLINE_THRESH) / (rep_max - BORDERLINE_THRESH + 1e-12))
...
cbar_edges.set_label("log10(|mean step| / (SD + 0.1) + 1)")
